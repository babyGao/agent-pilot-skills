...
| [huopan-listing-search](skills/huopan-listing-search/SKILL.md) | 把用户原话整句丢给火盘 MCP 即可查到真实商业地产房源（店铺 / 写字楼 / 厂房仓储 / 公寓 / 酒店），免握手免探测，返回结构化房源并渲染成 Markdown 卡片 | ✅ 可用 |
| [hello-boss](skills/hello-boss/SKILL.md) | 让 agent 去当你的销售 —— 把"谁需要我"挖到穷尽、从公开工商数据锁定经营主体和邮箱、写一封能拿到回复的开发信、批量发出 | ✅ 可用 |
...
- **一次调用，不用摸底**：端点、传输方式、可直接粘的请求体都写在 skill 里，第一个请求就是查询本身。服务无状态、无鉴权，不需要握手，也不需要先列工具，一个来回约 3~5 秒。
- **不用你拆字段**：把用户原话整句透传即可。服务端跑自己的模型拆出交易类型、业态、城市、地标、面积、预算，抽取质量不取决于调用平台，提示词改进也无需重发 skill。
- **知道库里到底有什么**：998 套、出售 748 / 出租 250、上海占大头、写字楼最多——都以事实给出，模型能分清"结果本来就少"和"调用失败了"。
- **出卡片，不出 JSON**：一份 Markdown 表格模板（左列项目名、右列详情），任何平台都渲染得出卡片列表；配一张返回字段表，把经常缺失的字段（面积、价格——源数据本来就没有）标了出来。
- **查不到也说得清**：0 命中时返回里带上放宽建议的数字（各放宽一个条件分别能出多少套）与在库统计，模型据此说明库里覆盖什么、建议往哪个方向放宽，而不是干巴巴一句"没找到"。
...
## hello-boss — 让 agent 去当你的销售

### 问题在哪

冷启动获客真正难的是两头，而现成工具几乎只管中间。**够不到人**：问一个做物流的"客户是谁"，
答案永远是"货运同行"——因为没人有办法系统地穷举"谁能买我这套东西"。就算拿到了名字，也不知道
这个品牌背后是哪个法人主体在经营：工商数据里满是同名公司、代运营、已注销的壳，还有代工厂冒充
经营方。**送不到人**：信退回来，或者进了垃圾箱。

### 你会得到什么

- **需求挖掘的五条穷举路径** —— skill 先逼你把身份换成**作业**（不是"我是物流公司"，而是
  "我把有体积有重量的东西从 A 搬到 B，并承担时效和破损风险"），然后用**投入产出表的直接消耗系数**
  走产业链、逐个过**20 个行业门类**防漏、把同一作业迁移到别的行业、**把招聘 JD 当需求信号读**
  （挂 5 个数据录入岗，说明量大到要雇人、而且预算已批）、以及从"我解除什么约束"反推。
  典型效果是候选行业从 4 个变成 30 个以上。
- **排序有依据，不凭感觉** —— 采购强度取自投入产出表，邮箱覆盖率**跑 10 家小样本量出来**。
  覆盖率 20% 的行业，挖 100 家只能发 20 封；这个数决定做不做，估错了后面全白干。
- **站得住的主体判定** —— 四条互相独立的信号（品牌搜企业名、商标持有人、地址吻合、店铺层级）
  收敛成三档置信度。弱档一律不猜，改走兜底通道直接问"贵司市场对接的邮箱是哪个"。
- **一封为拿回复而写的信，不是为交付而写** —— 四段、200~250 字、逐家只换一个变量，
  外加一张"写了就没人回"的措辞表。我们真为一轮投放写过 58 段一店一写的深度论证段，
  全部弃用——skill 里讲清了为什么。
- **三份验收物 + 固定目录** —— 需求调研报告、合作对象清单、开发信模板，路径不许挪动。
  不在这张表里的东西不做。
- **一次 58 家真实投放的实测数字** —— 频控阈值、退信率、按收件域名的送达差异，
  全在 `references/field-notes.md` 里，**包括我们做错的部分**。
...
不需要 MCP server，也不需要额外运行时——它是一份参考 + 流程型 skill。
第四步把清单和模板交给你现有的发信程序即可。

### 怎么用

只要你提到找客户、拓客、对象清单、开发信，agent 会自己加载。也可以显式触发——
注意四步**不是必须从头走**，从中间进入也可以：

> "我是做物流的，帮我把能服务的行业挖一遍，出调研报告。"
>
> "把这份名单整理成合格的对象清单。" · "看看这封开发信有什么毛病。"
